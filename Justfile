...
# Build with specific extras: just build-extras voice anthropic mcp
build-extras +EXTRAS:
    #!/usr/bin/env bash
    set -euo pipefail
    # `+EXTRAS` captures positional args as a space-separated string.
    # Quote each arg defensively to neutralize shell metacharacters.
    extras=""
    for e in {{EXTRAS}}; do
      # Strict allowlist — extras names are upstream-controlled identifiers,
      # not arbitrary user input. Reject anything outside [a-z0-9_-].
      case "$e" in
        *[!a-z0-9_-]*)
          echo "rejected extra name with metacharacters: $e" >&2
          exit 2
          ;;
      esac
      extras+="\"$e\" "
